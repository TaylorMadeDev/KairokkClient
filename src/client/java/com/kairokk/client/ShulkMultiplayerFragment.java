...
public final class ShulkMultiplayerFragment extends Fragment {
	private static final int CONTENT_WIDTH_DP = 760;
	...
	@Override public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable DataSet state) {
		...
		FrameLayout.LayoutParams contentParams = new FrameLayout.LayoutParams(ShulkServerUi.px(context, CONTENT_WIDTH_DP),
	...
	private void loadServers() {
		...
		for (int i = 0; i < servers.size(); i++) {
			...
			LinearLayout.LayoutParams params = ShulkServerUi.full(requireContext(), 66, 6);
	...
	private void updatePing(ServerRow row) {
		Core.executeOnUiThread(() -> {
			...
			String motd = row.data.motd == null ? "" : row.data.motd.getString().replace('\n', ' ').replaceAll("(?i)§[0-9A-FK-ORX]", "");
			if (motd.length() > 42) motd = motd.substring(0, 42) + "…";
			row.detail.setText(row.data.ip + "  ·  " + result + (motd.isBlank() ? "" : "  ·  " + motd));
		});
	}
	...
	private final class ServerRow {
		final int index; final ServerData data; final LinearLayout view; final TextView detail;
		ServerRow(Context context, int index, ServerData data) {
			this.index = index; this.data = data; view = new LinearLayout(context); view.setOrientation(LinearLayout.VERTICAL); view.setGravity(Gravity.CENTER_VERTICAL);
			...
			TextView name = ShulkServerUi.text(context, data.name, 20, 0xFFF1ECEF); name.setGravity(Gravity.LEFT | Gravity.CENTER_VERTICAL);
			detail = ShulkServerUi.text(context, data.ip, 13, 0xFF817984); detail.setGravity(Gravity.LEFT | Gravity.CENTER_VERTICAL);
			view.addView(name, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ShulkServerUi.px(context, 29)));
			view.addView(detail, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ShulkServerUi.px(context, 23)));
