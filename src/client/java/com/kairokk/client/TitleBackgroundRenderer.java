...
public final class TitleBackgroundRenderer {
	...
	public static void render(GuiGraphicsExtractor graphics, int width, int height) {
		...
		graphics.fillGradient(0, 0, width, height, 0xFF080F1D, 0xFF050912);
		TitleOrbRenderer.drawSoftOrb(graphics, width / 2, (int) (height * 0.35F),
			Math.round(shortestSide * 0.14F), 0x164F9A, 50);
		...
		for (Orb orb : ORBS) {
			...
			TitleOrbRenderer.drawSoftOrb(graphics, Math.round(x), Math.round(y), radius, blue, orb.alpha);
		}
	}
