...
public final class KairokkBackendGateScreen extends Screen {
	...
	@Override public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float deltaTick) {
		...
		boolean linked = KairokkClient.realtime() != null && KairokkClient.realtime().isLinkConfigured();
		graphics.centeredText(Minecraft.getInstance().font, linked ? "A secure dashboard connection is required before continuing." : "Link this client to your dashboard once before continuing.", centerX, centerY + 3, 0xFFA8BBD4);
		...
		graphics.centeredText(Minecraft.getInstance().font, linked ? "Retry connection" : "Dashboard link required", centerX, retryTop + 10, 0xFFF4F8FF);
		graphics.centeredText(Minecraft.getInstance().font, linked ? "The client refreshes its secure link automatically." : "Run linkKairokkDashboard once; your password is never stored.", centerX, centerY + 104, 0xFF7C93B0);
	}
